...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const rows = await sql`
        SELECT locale, content, is_published, updated_at FROM curriculum_entries ORDER BY locale
      `
      ...
      console.error(e)
      res.status(500).json({ error: 'Failed to load curriculum entries' })
    }
    return
  }
  ...
  if (req.method === 'PUT') {
    ...
    try {
      await sql`
        INSERT INTO curriculum_entries (locale, content, is_published, updated_at)
      ...
      `
      const rows = await sql`
        SELECT locale, content, is_published, updated_at FROM curriculum_entries WHERE locale = ${locale}
      `
      const r = rows[0]
      res.status(200).json({
        locale: r.locale,
